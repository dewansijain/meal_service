    ...
    <div className='fixed top-0 right-0 left-0 bg-white z-50'>
      ...
      <div className="flex flex-row justify-between items-center px-5 lg:px-20 py-4 shadow-md">
        ...
        <div className=" hidden lg:flex flex-col lg:flex-row gap-10 items-center">
          ...
          <Link activeClass='active' spy={true} to={menuItems[4].link} smooth={true} duration={1000} offset={-90} className="text-xl cursor-pointer hover:text-red-700 transition-all duration-00">{menuItems[4].name}</Link>
        </div>
      ...
      <AnimatePresence className="overflow-x-hidden">
        {show && (
          <motion.div
            ...
            <Link to={menuItems[0].link} smooth={true} duration={1000} offset={-100} className="text-xl cursor-pointer hover:text-red-700 transition-all duration-00">{menuItems[0].name}</Link>
            <Link to={menuItems[1].link} smooth={true} duration={1000} className="text-xl cursor-pointer hover:text-red-700 transition-all duration-00">{menuItems[1].name}</Link>
            <Link to={menuItems[2].link} smooth={true} duration={1000} className="text-xl cursor-pointer hover:text-red-700 transition-all duration-00">{menuItems[2].name}</Link>
            <Link to={menuItems[3].link} smooth={true} duration={1000} className="text-xl cursor-pointer hover:text-red-700 transition-all duration-00">{menuItems[3].name}</Link>
            <Link to={menuItems[4].link} smooth={true} duration={1000} className="text-xl cursor-pointer hover:text-red-700 transition-all duration-00">{menuItems[4].name}</Link>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
